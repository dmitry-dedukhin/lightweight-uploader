...
namespace uploader
{
	...
	public class Constants
	{
		...
		public readonly static int JsNotifyTryies = 100;
	...
	public class FileUpload : INotifyPropertyChanged
	{
		...
		public FileUpload(Dispatcher dispatcher, FileInfo fileToUpload)
			: this(dispatcher)
		{
			File = fileToUpload;
			...
			if (sf != null && sf.SessionId != null && sf.UploadUrl != null)
			{
				...
				UploadUrl = new Uri(sf.UploadUrl, UriKind.Absolute);
			}
		}
		...
		{
			ErrorDescr += where + "[" + UploadUrl.ToString() + "](" + BytesUploaded + "/" + FileLength + "){" + ex.Message + " / " + ex.InnerException.Message + " / STACK: " + ex.StackTrace + "}";
		}
